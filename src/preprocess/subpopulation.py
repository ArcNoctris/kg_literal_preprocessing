...
def subpopulation_binning(data: Data, num_bins=10, use_lof=False, bound_approach="r", **kwargs):
    ...
    for relation in relevent_relations:
        ...
        if bound_approach =="rv":
            ...
            children:List[Tensor] = rv_bound_child_extraction(data,relation)
        ...
        for i in range(len(children)):
            ...
            if (use_lof):
                ...
                if len(outliers) > 0:
                    ...
                    data.r2i[f'{URI_PREFIX}predicat#outlier-{relation}-{i}'] = data.num_relations

                    data.i2e.append(
                        (f'{URI_PREFIX}entitys#outlier-{relation}-{i}', f'{URI_PREFIX}outlier'))
                    data.e2i[(f'{URI_PREFIX}entitys#outlier-{relation}-{i}',
                    ...
                    predicat_mapping = np.vectorize(
                        lambda t: data.r2i[f'{URI_PREFIX}predicat#outlier-{relation}-{i}'])
...
def rv_bound_child_extraction(data:Data,relation:int)-> List[Tensor]:
    triples = data.triples[data.triples[:,1]==relation]
    parent = triples[:,0]
...
def rel_bound_LOF_subpopulation(data:Data, num_bins=10,**kwargs):
    return subpopulation_binning(data,num_bins=num_bins,use_lof=True ,bound_approach='r')
def rel_val_bound_LOF_subpopulation(data:Data, num_bins=10,**kwargs):
    return subpopulation_binning(data,num_bins=num_bins,use_lof=True, bound_approach='rv')
