...
def bin_numbers(data: Data, num_bins=3, use_lof=False, num_bins_as_percent=False, equal_height_binning=False, **kwargs):
    ...
    for relation in relevent_relations:
        ...
        if (use_lof):
            ...
            outliers = sub_df[(outlier_scores <= threshold) & (outlier_scores<-1)]
            sub_df = sub_df[(outlier_scores > threshold) | (outlier_scores>=-1)]
            outlier_df = augmented_df[(outlier_scores <= threshold) & (outlier_scores<-1)].clone()
            augmented_df = augmented_df[(outlier_scores > threshold) | (outlier_scores>=-1)]
            if len(outliers)>0:
                ...
                data.e2i[(f'{URI_PREFIX}entitys#outlier-{relation}', f'{URI_PREFIX}outlier')] = data.num_entities

                data.num_relations += 1
                data.num_entities += 1

                object_mapping = np.vectorize(lambda t: data.e2i[(f'{URI_PREFIX}entitys#outlier-{relation}', f'{URI_PREFIX}outlier')])

                predicat_mapping = np.vectorize(
                    lambda t: data.r2i[f'{URI_PREFIX}predicat#outlier-{relation}'])

                
                # outlier_df = augmented_df[torch.isin(augmented_df[:,0], outliers[:,0].to(torch.int32))].clone()
                # augmented_df = augmented_df[torch.isin(augmented_df[:,0], sub_df[:,0].to(torch.int32))]
                # outlier_df = augmented_df[(outlier_scores <= threshold) & (outlier_scores<-1)].clone()
                # augmented_df = augmented_df[(outlier_scores > threshold) | (outlier_scores>=-1)]

                outlier_df[:, 1] = torch.tensor(np.array([predicat_mapping(outliers[:, 0])]), dtype=torch.int32)
                outlier_df[:, 2] = torch.tensor(np.array([object_mapping(outliers[:, 0])]), dtype=torch.int32)
        ...
        augmented_df[:, 1] = torch.tensor(np.array([predicat_mapping(sub_df[:, 2])]), dtype=torch.int32)
        augmented_df[:, 2] = torch.tensor(np.array([object_mapping(sub_df[:, 2])]), dtype=torch.int32)
